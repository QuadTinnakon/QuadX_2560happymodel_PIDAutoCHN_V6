...
void mpu6050_Gyro_Values()
{
  ...
  Wire.endTransmission();
    gyroRaw[YAXIS] = ((result[0] << 8) | result[1]);//-12     -3
    gyroRaw[XAXIS] = ((result[2] << 8) | result[3]);//37      15
...
void mpu6050_Accel_Values()
{
  ...
  Wire.endTransmission();
    accelRaw[YAXIS] = ((result[0] << 8) | result[1])*-1;//+ 105 + 100 max=4054.46, min=-4149.48
    accelRaw[XAXIS] = ((result[2] << 8) | result[3])*-1;// - 45 - 35 max=4129.57, min=-4070.64
...
void Mag5883Read()
{
 ...
 int i = 0;
  ...
  Wire.endTransmission();   
  MagX = ((result[0] << 8) | result[1])*-1;//offset + 1.05
  MagZ = ((result[2] << 8) | result[3])*-1;// + 0.05
  MagY = ((result[4] << 8) | result[5]);// - 0.55
...
void sensor_Calibrate()
{
  Serial.print("Sensor_Calibrate");Serial.println("\t");
    ...
    acc_offsetX = 0.43;//-0.18 0.11 -0.36  Trim PITCH CONTROL   -10.07	-10.55	-9.82
    acc_offsetY = -0.21;//0.16 -0.14 0.18 Trim ROLL CONTROL     10.39	9.74	11
